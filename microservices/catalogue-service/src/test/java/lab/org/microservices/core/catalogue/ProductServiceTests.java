...
public class ProductServiceTests extends MongoDbTestBase {
    ...
    void listProducts() {
        ...
        sendCreateProductEvent(1);
        sendCreateProductEvent(2);
        assertNotNull(repository.findByProductId(1).block());
    ...
    void getProductInvalidParameterString() {
        getAndVerifyProduct("/no-integer", BAD_REQUEST)
                .jsonPath("$.path").isEqualTo("/no-integer")
    ...
    private WebTestClient.BodyContentSpec getAndVerifyProduct(int productId, HttpStatus status) {
        return getAndVerifyProduct("/product/" + productId, status);
